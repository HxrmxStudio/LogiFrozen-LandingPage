import React from "react";
import { motion } from "framer-motion";
import { quotes } from "../assets";
import { fadeIn } from "../util/motion";

function FeedbackCard({ content, name, title, img, index }) {
    return (
        <motion.div
            variants={fadeIn("up", "spring", index * 0.2, 1)} // Animación con retraso único
            className="flex justify-between flex-col px-10 py-12 rounded-[20px] max-w-[370px] md:mr-10 sm:mr-5 mr-0 my-5 feedback-card"
        >
            {/* Imagen de comillas decorativas */}
            <img src={quotes} alt="Comillas dobles" className="w-[42px] h-[27px] object-contain" />

            {/* Testimonio */}
            <p className="font-poppins font-normal text-[18px] leading-[32px] text-white my-10">
                {content}
            </p>

            {/* Información del cliente */}
            <div className="flex flex-row">
                <img
                    src={img}
                    alt={`Foto de ${name}`}
                    className="w-[48px] h-[48px] rounded-full"
                />
                <div className="flex flex-col ml-4">
                    <h4 className="font-poppins font-semibold text-[20px] leading-[32px] text-white">
                        {name}
                    </h4>
                    <p className="font-poppins font-normal text-[16px] leading-[24px] text-dimWhite">
                        {title}
                    </p>
                </div>
            </div>
        </motion.div>
    );
}

export default FeedbackCard;
